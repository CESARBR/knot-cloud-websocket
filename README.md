...
Update the property metadata of device with `id` with the object `metadata` on KNoT Cloud. When successful emits a `metadata` message.
...
async function main() {
  client.on('ready', () => {
    client.update('7e133545550e496a', {
      room: {
