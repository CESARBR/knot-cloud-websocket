...
Update the property metadata of device with `id` with the object `metadata` on KNoT Cloud. When successful emits a `updated` message.
...
Remove the device with `id` on KNoT Cloud. When successful emits a `unregistered` message.
...
- `schema` **Array** a set of properties (associated to KNoT protocol) with details about sensors/actuators.
  * `sensor_id` **Number** a sensor id between 0 and 255.
  * `value_type` **Number** sensor type based on KNoT protocol.
  * `unit` **Number** sensor unit based on KNoT protocol.
